...
"""Diff computation for snapshots — files, pre_commands, command output."""
...
def _collect_snapshot_files(profile: dict, root: Path) -> dict[str, str]:
    ...
    for fp in filepaths:
        if not validate_path(fp, root):
            logger.warning("Path traversal attempt in snapshot collection, skipping file")
            continue
        try:
            content = fp.read_text(encoding="utf-8")
        except (OSError, UnicodeDecodeError):
...
def _build_current_files(profile: dict, exclude: list[str], root: Path) -> dict[str, str]:
    ...
    for fp in current_filepaths:
        if not validate_path(fp, root):
            logger.warning("Path traversal attempt in diff, skipping file")
            continue
        try:
            content = fp.read_text(encoding="utf-8")
        except (OSError, UnicodeDecodeError):
...
def _read_file_from_disk(path, root=None):
    ...
    if root is not None and not validate_path(fp, root):
        logger.warning("Path traversal attempt in repo-map, skipping file")
        return None
    if not fp.is_file():
        return None
    try:
