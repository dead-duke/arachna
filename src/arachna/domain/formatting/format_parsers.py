"""Format parsers — header/block parsers and language dispatch, separated to break circular imports."""

import ast as _ast
import re

from .format_language import C_LIKE_LANGS, SCRIPT_LANGS

_RE_PY_IMPORT_FROM = re.compile(r"^from\s+([\w\.]+)\s+import", re.MULTILINE)
_RE_PY_MULTILINE_IMPORT = re.compile(r"^import\s*\(([^)]*)\)", re.MULTILINE)
...
def _build_header_parsers() -> dict:
    parsers = {"python": _parse_python, "gdscript": _parse_c_like}
    for lang in C_LIKE_LANGS:
        parsers[lang] = _parse_c_like
    for lang in SCRIPT_LANGS:
        parsers[lang] = _parse_script
    return parsers
