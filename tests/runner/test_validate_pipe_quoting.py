...
def test_pipe_inside_single_quotes_safe():
    ...
    is_safe, reason = _validate_command("tree -I '*.pyc|*.egg-info' src")
...
def test_pipe_inside_double_quotes_safe():
    ...
    is_safe, reason = _validate_command('grep "error|warning" log.txt')
    assert is_safe, f"Expected safe, got: {reason}"
...
def test_actual_pipe_still_detected():
    ...
    is_safe, reason = _validate_command("cat file.txt | grep pattern")
    assert is_safe, f"Expected safe, got: {reason}"


def test_actual_pipe_with_unknown_command():
    """Real pipe with unknown command should be blocked."""
    is_safe, reason = _validate_command("cat file.txt | unknown_cmd_xyz")
    assert not is_safe
