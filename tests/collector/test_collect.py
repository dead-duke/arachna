...
def test_merge_mode_single_part(tmp_path, monkeypatch):
    ...
    created1 = collect(profile, "P", "out", merge=True)
    assert len(created1) == 1
    assert "c_1.md" in created1[0]

    created2 = collect(profile, "P", "out", merge=True)
...
def test_merge_mode_multiple_parts(tmp_path, monkeypatch):
    ...
    assert "c_1.md" in created1[0]
    assert "c_2.md" in created1[1]

    created2 = collect(profile, "P", "out", merge=True)
    assert len(created2) == 2
    assert "c_3.md" in created2[0]
    assert "c_4.md" in created2[1]
