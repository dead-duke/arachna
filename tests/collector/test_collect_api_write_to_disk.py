...
def test_collect_api_write_to_disk_false(tmp_path):
    ...
    result = collect(root=tmp_path, profile=profile, output_dir="out", write_to_disk=False)
...
def test_collect_api_parts_match_files(tmp_path):
    ...
    result = collect(root=tmp_path, profile=profile, output_dir="out", write_to_disk=True)
    assert len(result.parts) == 1
