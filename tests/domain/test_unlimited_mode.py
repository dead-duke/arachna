"""Tests for unlimited mode (max_tokens=-1) — split, split_sections, pack_into_parts, collect, dry_run."""
...
def test_unlimited_split_by_file():
    content = "### a.py\n\n```python\ncode\n```\n\n### b.py\n\n```python\ncode\n```"
    parts = split(content, -1, mode="by_file")
    assert len(parts) == 1
...
def test_unlimited_split_by_paragraph():
    parts = split("para1\n\npara2\n\npara3", -1, mode="by_paragraph")
    assert len(parts) == 1


def test_unlimited_split_single():
    parts = split("hello world", -1, mode="single")
    assert len(parts) == 1
    assert "hello world" in parts[0]
