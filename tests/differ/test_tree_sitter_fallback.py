...
def test_has_tree_sitter_for_none_installed():
    ...
    with (
        patch("arachna.differ_structural._HAS_TS_JS", False),
        patch("arachna.differ_structural._HAS_TS_TS", False),
        patch("arachna.differ_structural._HAS_TS_GO", False),
    ):
...
def test_has_tree_sitter_for_js_installed():
    with (
        patch("arachna.differ_structural._HAS_TS_JS", True),
        patch("arachna.differ_structural._HAS_TS_TS", False),
        patch("arachna.differ_structural._HAS_TS_GO", False),
    ):
...
def test_has_tree_sitter_for_ts_installed():
    with (
        patch("arachna.differ_structural._HAS_TS_JS", False),
        patch("arachna.differ_structural._HAS_TS_TS", True),
        patch("arachna.differ_structural._HAS_TS_GO", False),
    ):
...
def test_structural_diff_js_fallback_warning():
    ...
    with (
        patch("arachna.differ_structural._has_tree_sitter_for", return_value=False),
    ):
