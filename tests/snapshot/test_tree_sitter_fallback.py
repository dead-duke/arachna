...
def test_has_tree_sitter_for_none_installed():
    with (
        patch("arachna.snapshot.differ_structural._HAS_TS_JS", False),
        patch("arachna.snapshot.differ_structural._HAS_TS_TS", False),
        patch("arachna.snapshot.differ_structural._HAS_TS_GO", False),
    ):
...
def test_has_tree_sitter_for_js_installed():
    with (
        patch("arachna.snapshot.differ_structural._HAS_TS_JS", True),
        patch("arachna.snapshot.differ_structural._HAS_TS_TS", False),
        patch("arachna.snapshot.differ_structural._HAS_TS_GO", False),
    ):
...
def test_has_tree_sitter_for_ts_installed():
    with (
        patch("arachna.snapshot.differ_structural._HAS_TS_JS", False),
        patch("arachna.snapshot.differ_structural._HAS_TS_TS", True),
        patch("arachna.snapshot.differ_structural._HAS_TS_GO", False),
    ):
...
def test_structural_diff_js_fallback_warning():
    old = "function foo() {\n    return 1;\n}\n"
    new = "function foo() {\n    return 2;\n}\n"
    with patch("arachna.snapshot.differ_structural._has_tree_sitter_for", return_value=False):
