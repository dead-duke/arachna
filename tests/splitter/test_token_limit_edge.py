...
def test_exact_max_tokens_single_part(tmp_path):
    ...
    # 100 tokens with default tokenizer = 400 characters
    content = "x" * 400
...
def test_one_token_over_limit_two_parts(tmp_path):
    ...
    # Will try to split on paragraphs — single paragraph is too large
    # by_paragraph splits on \n\n, single block goes as-is
    assert len(parts) >= 1
...
def test_single_mode_one_over_truncates():
    """Single mode one token over limit — truncates."""
    content = "hello"
    tokens = count_tokens(content)
    parts = split(content, max_tokens=tokens - 1, mode="single")
...
def test_by_file_section_exact_fit(tmp_path):
    ...
    # Build two sections with ### marker
    a_content = "x" * 200  # 50 tokens
    b_content = "y" * 200  # 50 tokens
    content = f"### a.py\n\n```python\n{a_content}\n```\n\n### b.py\n\n```python\n{b_content}\n```"
...
def test_by_file_section_one_token_over(tmp_path):
    ...
    # Should split into 2 parts since combined exceeds by 1 token
    assert len(parts) == 2
