...
from arachna.splitter import pack_into_parts, split, split_sections
from arachna.tokenizer import count_tokens


def test_pack_into_parts_unlimited():
    sections = ["section1", "section2", "section3"]
    parts, indices = pack_into_parts(sections, 0, tokenizer=count_tokens)
    assert len(parts) == 1
...
def test_split_sections_unlimited():
    sections = ["a", "b", "c"]
    parts, indices = split_sections(sections, 0, tokenizer=count_tokens)
    assert len(parts) == 1
    assert indices[0] == [0, 1, 2]
