...
def test_doctor_project_root_not_dir(tmp_path, monkeypatch):
    """Doctor handles non-directory project_root."""
...
def test_doctor_gitignore_os_error(tmp_path, monkeypatch):
    """Doctor handles OSError when loading gitignore."""
    monkeypatch.chdir(tmp_path)
    (tmp_path / "src").mkdir()
