"""Integration tests for --snapshot update with pre_commands (v2.9.2)."""
...
def test_snapshot_update_pre_commands_not_blocked(tmp_path, monkeypatch):
    """--snapshot update runs pre_commands without blocked warnings (allow_file_args fix)."""
...
def test_diff_with_pre_commands_not_blocked(tmp_path, monkeypatch):
    """--diff runs current pre_commands without blocked warnings."""
    monkeypatch.chdir(tmp_path)
    src = tmp_path / "src"
    src.mkdir()
    (src / "main.py").write_text("print('hello')")
