...
def test_watcher_update_snapshot_replaces_files(tmp_path, setup_config, make_profile):
    ...
    profile = make_profile("src", ["*.py"])
    create_snapshot(profile, name="w-update", root=root)

    (src / "a.py").write_text("second")
    (src / "b.py").write_text("new file")

    update_snapshot("w-update", root=root, profile=profile)

    manifest = load_snapshot("w-update", root=root)
...
def test_watcher_update_snapshot_uses_existing_profile(tmp_path, setup_config, make_profile):
    ...
    profile = make_profile("src", ["*.py"])
    create_snapshot(profile, name="w-auto-profile", root=root)

    (src / "b.py").write_text("new")

    update_snapshot("w-auto-profile", root=root)

    manifest = load_snapshot("w-auto-profile", root=root)
    assert len(manifest["files"]) == 2
