...
def test_cross_snapshot_diff_modified(tmp_path, monkeypatch):
    """Cross-snapshot diff detects modified file between two snapshots."""
...
def test_cross_snapshot_diff_added(tmp_path, monkeypatch):
    """Cross-snapshot diff detects file added in second snapshot."""
    monkeypatch.chdir(tmp_path)
    src = tmp_path / "src"
    src.mkdir()
...
def test_cross_snapshot_diff_deleted(tmp_path, monkeypatch):
    """Cross-snapshot diff detects file deleted in second snapshot."""
    monkeypatch.chdir(tmp_path)
    src = tmp_path / "src"
    src.mkdir()
...
def test_get_content_from_manifest(tmp_path, monkeypatch):
    """_get_content_from_manifest reads file content by hash."""
    monkeypatch.chdir(tmp_path)
...
def test_build_current_files(tmp_path, monkeypatch):
    """_build_current_files collects files from profile."""
    monkeypatch.chdir(tmp_path)
    src = tmp_path / "src"
    src.mkdir()
...
def test_diff_file_sets_modified(tmp_path, monkeypatch):
    """_diff_file_sets detects modifications."""
...
def test_diff_file_sets_added():
    """_diff_file_sets detects added files."""
    old_files = {}
...
def test_diff_file_sets_deleted():
    """_diff_file_sets detects deleted files."""
