...
from arachna.store import _collect_referenced_hashes


def test_collect_referenced_hashes_files():
    """Extracts hashes from files field."""
    manifests = [
        {
            "files": {
                "a.py": "sha256:abc123",
...
def test_collect_referenced_hashes_pre_commands():
    """Extracts hashes from pre_commands field."""
    manifests = [
        {
            "files": {},
            "pre_commands": {
                "pre: echo": "sha256:cmd111",
            },
        }
    ]
    hashes = _collect_referenced_hashes(manifests)
...
def test_collect_referenced_hashes_command():
    """Extracts hashes from command field."""
    manifests = [
        {
            "files": {},
            "command": {
                "command output": "sha256:out222",
            },
        }
    ]
    hashes = _collect_referenced_hashes(manifests)
...
def test_collect_referenced_hashes_all_fields():
    """Extracts hashes from files, pre_commands, and command together."""
    manifests = [
        {
...
def test_collect_referenced_hashes_duplicates():
    """Duplicate hashes across manifests — deduplicated."""
    manifests = [
